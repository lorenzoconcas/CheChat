function init(){
    ...
    if(isMobile){
         if (path === "/register")
            $("#login_box").css("height", "1280px");
        else if (path === "/")
            $("#login_box").css("height", "720px");
    }
    else{
        if (path === "/register")
            $("#login_box").css("height", "512px");
        else if (path === "/")
            $("#login_box").css("height", "310px");
    }
    let login_top = ($(window).height() - $("#login_box").height()) / 2;
    console.log(login_top);
    $("#login_box").css("top", login_top);
...
function loadMobileCSS(){
	  var head = document.getElementsByTagName('head')[0];
        ...
        link.rel = 'stylesheet';
        link.type = 'text/css';
        link.href = 'static/chat/css/login_mobile.css';
        link.media = 'all';
